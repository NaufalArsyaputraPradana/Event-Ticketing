...
@section('title', 'Edit Rekening Bank - Admin')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Edit Rekening Bank</h1>
                <p class="text-gray-600">Edit informasi rekening bank</p>
            </div>
        ...
        <form action="{{ route('admin.bank-accounts.update', $bankAccount) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="account_type" id="account_type" required
                        ...
                        <option value="savings" {{ old('account_type', $bankAccount->account_type) === 'savings' ? 'selected' : '' }}>Tabungan</option>
                        <option value="current" {{ old('account_type', $bankAccount->account_type) === 'current' ? 'selected' : '' }}>Giro</option>
                    </select>
                    @error('account_type')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="mt-6">
                ...
                <textarea name="description" id="description" rows="3"
                          ...
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500">{{ old('description', $bankAccount->description) }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-6">
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ $bankAccount->is_active ? 'checked' : '' }}
            ...
            <div class="flex justify-end mt-8">
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-2"></i>Update Rekening Bank
                </button>
            </div>
        </form>
    </div>
